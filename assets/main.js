...
/* Search overlay open/close is handled in header-search-header.jinja (search-popup-overlay). */
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  $('.search-input-input').on('input', function (event) {
    if ($(event.currentTarget).closest('.search-popup-autocomplete').length) {
      return;
    }
...
function fetchProductsSearch(inputEl, catId, query) {
  var $items = inputEl
    ? $(inputEl).closest('.autocomplete').find('.autocomplete-items')
    : $('.autocomplete-items');
  var isPopupSearch = inputEl
    ? $(inputEl).closest('.search-popup-autocomplete').length > 0
    : false;
    ...
    .then(function (response) {
      if (response && response.results) {
        ...
        for (var i = 0; i < response.results.length; i++) {
          ...
          $items.append(isPopupSearch ? renderPopupSearchResult(product) : renderDefaultSearchResult(product));
        }
      }
    });
}

function renderDefaultSearchResult(product) {
  return '<div><a href="' + escapeSearchHtml(product.html_url || '#') + '">' + escapeSearchHtml(product.name || '') + '</a></div>';
}

function renderPopupSearchResult(product) {
  ...
  var oldPrice = product.formatted_sale_price && product.formatted_price
    ? '<span class="search-popup-product-result__old-price">' + escapeSearchHtml(product.formatted_price) + '</span>'
    : '';
  var image = imageUrl
    ? '<img class="search-popup-product-result__image" src="' + escapeSearchHtml(imageUrl) + '" alt="' + escapeSearchHtml(product.name || '') + '" loading="lazy">'
  ...
  return (
    '<div class="search-popup-product-result">' +
      '<a class="search-popup-product-result__link" href="' + escapeSearchHtml(product.html_url || '#') + '">' +
        image +
        '<span class="search-popup-product-result__content">' +
          '<span class="search-popup-product-result__title">' + escapeSearchHtml(product.name || '') + '</span>' +
          '<span class="search-popup-product-result__prices">' +
            '<span class="search-popup-product-result__price">' + escapeSearchHtml(price) + '</span>' +
            oldPrice +
          '</span>' +
        '</span>' +
      '</a>' +
